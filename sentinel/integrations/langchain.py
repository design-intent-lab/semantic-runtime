...
class SentinelCallbackHandler:
    """Sentinel OS integration for LangChain agents."""

    def __init__(self, api_key: str):
        self.api_key = api_key
        self.risk_threshold = 0.7  # Actions with risk > 0.7 are blocked

    def verify_action(self, action_type: str, payload: dict, context: Optional[dict] = None) -> dict:
        """
    ...
    def __call__(self, action_type: str, payload: dict, context: Optional[dict] = None):
        """Called by LangChain during agent execution."""
        decision = self.verify_action(action_type, payload, context)
        if decision["decision"] == "deny":
            raise ValueError(f"Sentinel blocked action: {decision['reason']}")
        return decision